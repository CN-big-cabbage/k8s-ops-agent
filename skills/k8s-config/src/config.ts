...
async function handleK8sConfig(params: K8sConfigParams, pluginConfig?: PluginConfig): Promise<string> {
  try {
...
export function registerK8sConfigTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_config",
    description: "Kubernetes ConfigMap and Secret operations: list, describe, get data, create, update, delete",
    schema: K8sConfigSchema,
    handler: async (params: K8sConfigParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sConfig(params, pluginConfig);
    },
  });
}

export { K8sConfigSchema, handleK8sConfig };