...
type K8sCronJobParams = z.infer<typeof K8sCronJobSchema>;
...
export function registerK8sCronJobTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_cronjob",
    description:
      "Kubernetes CronJob operations: list, describe, status, suspend/resume, trigger manual run, job history",
    schema: K8sCronJobSchema,
    handler: async (params: K8sCronJobParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sCronJob(params, pluginConfig);
    },
  });
}
