...
async function handleK8sSvc(params: K8sSvcParams, pluginConfig?: any): Promise<string> {
  try {
    const { coreApi } = initializeK8sClient(
...
export function registerK8sSvcTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_svc",
    description: "Kubernetes Service operations: list, describe, endpoints, status",
    schema: K8sSvcSchema,
    handler: async (params: K8sSvcParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sSvc(params, pluginConfig);
    },
  });
}
