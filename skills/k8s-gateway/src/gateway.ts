...
type K8sGatewayParams = z.infer<typeof K8sGatewaySchema>;
...
export function registerK8sGatewayTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_gateway",
    description:
      "Gateway API management: list/describe gateways, list/describe routes, list classes, health status",
    schema: K8sGatewaySchema,
    handler: async (params: K8sGatewayParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sGateway(params, pluginConfig);
    },
  });
}
