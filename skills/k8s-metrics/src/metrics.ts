...
async function handleK8sMetrics(params: K8sMetricsParams, pluginConfig?: PluginConfig): Promise<string> {
  try {
...
export function registerK8sMetricsTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_metrics",
    description:
      "Kubernetes resource metrics: pod/node resource usage, top pods/nodes, namespace usage summary, cluster capacity report",
    schema: K8sMetricsSchema,
    handler: async (params: K8sMetricsParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sMetrics(params, pluginConfig);
    },
  });
}
