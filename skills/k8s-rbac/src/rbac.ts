...
type K8sRbacParams = z.infer<typeof K8sRbacSchema>;
...
export function registerK8sRbacTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_rbac",
    description:
      "Kubernetes RBAC operations: list/describe ServiceAccounts, Roles, ClusterRoles, Bindings; who_can check; audit overprivileged SAs",
    schema: K8sRbacSchema,
    handler: async (params: K8sRbacParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sRbac(params, pluginConfig);
    },
  });
}
