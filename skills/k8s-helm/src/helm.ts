...
type K8sHelmParams = z.infer<typeof K8sHelmSchema>;
...
export function registerK8sHelmTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_helm",
    description:
      "Helm release management: list, status, history, values, diff, rollback, uninstall",
    schema: K8sHelmSchema,
    handler: async (params: K8sHelmParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sHelm(params, pluginConfig);
    },
  });
}
