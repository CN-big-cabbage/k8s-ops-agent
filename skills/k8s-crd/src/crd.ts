...
type K8sCrdParams = z.infer<typeof K8sCrdSchema>;
...
export function registerK8sCrdTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_crd",
    description:
      "Kubernetes CRD operations: list/describe CRD definitions, list/describe/delete custom resource instances",
    schema: K8sCrdSchema,
    handler: async (params: K8sCrdParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sCrd(params, pluginConfig);
    },
  });
}
