...
type K8sSecurityParams = z.infer<typeof K8sSecuritySchema>;
...
export function registerK8sSecurityTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_security",
    description:
      "Kubernetes security audit: namespace scan with scoring, PSA compliance, secret audit, image audit, privileged pod detection",
    schema: K8sSecuritySchema,
    handler: async (params: K8sSecurityParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sSecurity(params, pluginConfig);
    },
  });
}
