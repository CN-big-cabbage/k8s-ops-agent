...
type K8sTroubleshootParams = z.infer<typeof K8sTroubleshootSchema>;
...
export function registerK8sTroubleshootTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_troubleshoot",
    description:
      "Intelligent troubleshooting: pod_not_ready, service_no_endpoints, node_not_ready, pvc_pending, deployment_stuck, diagnose",
    schema: K8sTroubleshootSchema,
    handler: async (params: K8sTroubleshootParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sTroubleshoot(params, pluginConfig);
    },
  });
}
