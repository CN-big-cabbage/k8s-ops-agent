...
type K8sYamlParams = z.infer<typeof K8sYamlSchema>;
...
export function registerK8sYamlTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_yaml",
    description:
      "YAML resource management: export clean YAML, dry-run validation, diff live vs target, apply, template generation",
    schema: K8sYamlSchema,
    handler: async (params: K8sYamlParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sYaml(params, pluginConfig);
    },
  });
}
