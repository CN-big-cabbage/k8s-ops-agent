...
type K8sStatefulSetParams = z.infer<typeof K8sStatefulSetSchema>;
...
export function registerK8sStatefulSetTools(api: OpenClawPluginApi) {
  api.tools.register({
    name: "k8s_statefulset",
    description:
      "Kubernetes StatefulSet operations: list, describe, status, scale, rollout (restart/undo), update-image",
    schema: K8sStatefulSetSchema,
    handler: async (params: K8sStatefulSetParams) => {
      const pluginConfig = api.getPluginConfig?.("k8s");
      return await handleK8sStatefulSet(params, pluginConfig);
    },
  });
}
